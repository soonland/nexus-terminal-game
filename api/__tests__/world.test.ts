...
const FALLBACK_NARRATIVE = '[World AI unavailable — operating in offline mode. Try basic commands.]'
...
beforeEach(() => { delete process.env['GEMINI_API_KEY']; vi.unstubAllGlobals() })
...
describe('POST /api/world — with API key', () => {
  ...
  it('returns structured WorldAIResponse when fetch succeeds', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      makeGeminiResponse(JSON.stringify(VALID_AI_JSON))
    ))
  ...
  it('caps traceChange to 0–5 range', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      makeGeminiResponse(JSON.stringify(overLimit))
    ))
  ...
  it('clears newAccessLevel when accessGranted is false', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      makeGeminiResponse(JSON.stringify(withAccess))
    ))
