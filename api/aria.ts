...
const FALLBACK_REPLY = '...signal lost. try again.';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const trustScore = typeof body['trustScore'] === 'number' ? body['trustScore'] : 0;
    ...
    const systemPrompt =
      ...
      `The player's current trust score with you is ${String(trustScore)}/100. ` +
    ...
    const text = data.candidates?.[0]?.content?.parts?.[0]?.text?.trim() ?? FALLBACK_REPLY;
